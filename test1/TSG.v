...
(** Tree Substitution Grammar parser, 1st try.
    ...
    We define the TSG parser in terms of the corresponding non-weighted
    inference rules.  The parser is thus a simplified, symbolic variant 
    of the full ParTAGe parser.

    But... let's start with the CFG Earley parser!
*)
...
(*
Inductive rule (X:Type) : Type :=
  | Rule (head : X) (body : list X).
*)

Inductive rule : Type :=
  | Rule (head : non_terminal) (body : list symbol).


Definition head (r : rule) : non_terminal :=
  match r with
  | Rule x y => x
  end.
...
Definition lead (r : rule) : option symbol :=
  match r with
  | Rule _ (h :: t) => Some h
  | _ => None
  end.
...
(** Position in the input sentence is just a natural number.
    Ideally, it should be constrained to the list of actual
    positions between the words.
*)
Definition pos := nat.

Inductive cfg : dotted_rule -> pos -> pos -> Prop :=
  | cfg_ax (r : rule) (i : pos) : cfg r i i
  | cfg_sc i j r 
      (sent : pos -> option symbol)
      (* JW: eventually, [sent] should return [option terminal], but
         we would need to use the notion of a functor for that. *)
      (P: cfg r i j)
      (T: sent j = lead r) :
        cfg r i (S j).
  | 
